...
# Initialize tokenizer based on whether we're using pretrained models or not
if cfg.pretrained:
    # Use original tokenizers when using pretrained models
    ...
    # Build custom tokenizer from dataset (only train split) when training from scratch
    tokenizer = DatasetTokenizer(
        csv_file=cfg.ds_file,
        text_column='text',
        split='train',  # Only build vocabulary from train split
        max_length=512,
        min_freq=2,  # Only include tokens that appear at least 2 times
...
if (cfg.pretrained):
    ...
    if cfg.model_config.text_loss_type == 'mlm':
        ...
        modernbert_config.vocab_size = tokenizer.vocab_size
        text_encoder = ModernBertForMaskedLM(modernbert_config)
    else:
        gpt2_config = GPT2Config()
        gpt2_config.vocab_size = tokenizer.vocab_size
        text_encoder = GPT2LMHeadModel(gpt2_config)

# Create CLIP model with only contrastive loss
clip_model = CLIPModel(vision_encoder, text_encoder, cfg.model_config, eval_loaders)

wandb_logger = WandbLogger(project="bv-clip", name=cfg.wandb_name)
...
trainer.fit(clip_model, train_dl, val_dl)

test_result = trainer.test(clip_model, test_dl)
